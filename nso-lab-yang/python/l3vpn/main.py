...
class ServiceCallbacks(Service):
    ...
    def cb_create(self, tctx, root, service, proplist):
        ...
        for link in service.link:
            ...
            intfs = root.devices.device[link.pe.device].config.interface.GigabitEthernet
            for intf in intfs:
                if intf.id == link.pe.interface:
                    if not re.search(r"^L3VPN {}".format(service.vpn_name), intf.description) and not intf.shutdown:
                        raise Exception("Interface {} on {} must currently be shutdown!".format(link.pe.interface, link.pe.device))
                    break
